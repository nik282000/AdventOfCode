...
for p in puzzle: # Parsing input!
    if 'Button A:' in p:
        ...
        l.append(int(p[6:].strip().split()[0][2:-1]))
        l.append(int(p[6:].strip().split()[1][2:]))
        machines.append(l)

for m in machines: # Calculate the crossing point for the two lines
                   # Use Decimal() to help with floating point nonsence
                   # Use float() to help with Decimal nonsense
